...
Routine Description:
    ...
    This routine gets information for the given file.
...
Arguments:
    ...
    File - Supplies a pointer where a pointer to the new file information will
        be returned. The caller is responsible for calling the appropriate
        destroy routine for this structure.
...

{
    ...
    *File = NULL;
    ...
    //
    // Get the name portion of the filename (separated out from the path).
    //

    NameSize = strlen(FileName) + 1;
    LastSeparator = FileName + NameSize - 1;
    while ((NameSize > 1) && (*LastSeparator == '/')) {
        NameSize -= 1;
        LastSeparator -= 1;
    }
    ...
    if (NameSize <= 1) {
        Result = ENOENT;
        goto GetFileInformationEnd;
    }

    while ((LastSeparator != FileName) && (*LastSeparator != '/')) {
        LastSeparator -= 1;
    }

    NameSize -= (UINTN)LastSeparator - (UINTN)FileName;

    //
    // Now get the file information.
    //

    Result = SwStat(FileName, FALSE, &Stat);
    if (Result != 0) {
        Result = errno;
        goto GetFileInformationEnd;
    }
    ...
    //
    // Get the link destination if it's a link.
    //

    LinkBroken = FALSE;
    if (S_ISLNK(Stat.st_mode)) {
        if (SwStat(FileName, TRUE, &LinkStat) != 0) {
            LinkBroken = TRUE;
        }

        Result = SwReadLink(FileName, &LinkDestination);
        if (Result != 0) {
            goto GetFileInformationEnd;
        }
...
GetFileInformationEnd:
    if (LinkDestination != NULL) {
        free(LinkDestination);
    }

    if (Result != 0) {
        SwPrintError(Result, FileName, "Cannot access");
    }
