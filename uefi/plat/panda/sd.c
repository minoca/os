...
Abstract:
    ...
    This module implements PandaBoard SD support in UEFI.
...
#define EFI_SD_OMAP4_FROM_THIS(_BlockIo)                 \
        (EFI_SD_OMAP4_CONTEXT *)((VOID *)(_BlockIo) -    \
                        ((VOID *)(&(((EFI_SD_OMAP4_CONTEXT *)0)->BlockIo))))
...
#define EFI_SD_OMAP4_MAGIC 0x344F6453 // '4OdS'

#define EFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH_GUID              \
    {                                                       \
...
//
// Define the OMAP4 SD timeout in microseconds.
//

#define EFI_SD_OMAP4_TIMEOUT 1000000
...
Members:
    ...
    Magic - Stores the magic constand EFI_SD_OMAP4_MAGIC.
...
typedef struct _EFI_SD_OMAP4_CONTEXT {
    UINT32 Magic;
    EFI_HANDLE Handle;
...
} EFI_SD_OMAP4_CONTEXT, *PEFI_SD_OMAP4_CONTEXT;
...
typedef struct _EFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH {
    VENDOR_DEVICE_PATH DevicePath;
    UINT32 ControllerBase;
} EFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH, *PEFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH;

/*++

Structure Description:

    This structure defines the OMAP4 SD block I/O device path.
...
EFIAPI
EFI_STATUS
EfipSdOmap4FlushBlocks (
    EFI_BLOCK_IO_PROTOCOL *This
    );

EFI_STATUS
EfipSdOmap4ResetController (
    PEFI_SD_OMAP4_CONTEXT Device
    );
...
EFI_SD_OMAP4_CONTEXT EfiSdOmap4DiskTemplate = {
    EFI_SD_OMAP4_MAGIC,
    NULL,
    NULL,
    NULL,
...
EFI_SD_OMAP4_DEVICE_PATH EfiSdOmap4DevicePathTemplate = {
    {
        {
            {
                HARDWARE_DEVICE_PATH,
                HW_VENDOR_DP,
                sizeof(EFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH)
            },

            EFI_SD_OMAP4_BLOCK_IO_DEVICE_PATH_GUID,
        },
...

{
    ...
    UINT32 BlockSize;
    UINT32 ControllerBase;
    PEFI_SD_OMAP4_DEVICE_PATH DevicePath;
    PEFI_SD_OMAP4_CONTEXT Disk;
    EFI_SD_INITIALIZATION_BLOCK SdParameters;
    EFI_STATUS Status;

    ControllerBase = OMAP4430_HSMMC1_BASE;
    ...
    EfiCopyMem(Disk, &EfiSdOmap4DiskTemplate, sizeof(EFI_SD_OMAP4_CONTEXT));
    Disk->Handle = NULL;
    ...
    Status = EfiAllocatePool(EfiBootServicesData,
                             sizeof(EFI_SD_OMAP4_DEVICE_PATH),
                             (VOID **)&DevicePath);

    if (EFI_ERROR(Status)) {
        goto PandaEnumerateSdEnd;
    }

    EfiCopyMem(DevicePath,
               &EfiSdOmap4DevicePathTemplate,
               sizeof(EFI_SD_OMAP4_DEVICE_PATH));
    ...
    SdParameters.StandardControllerBase =
                 Disk->ControllerBase + SD_OMAP4_CONTROLLER_SD_REGISTER_OFFSET;
    ...
    SdParameters.FundamentalClock = SD_OMAP4_FUNDAMENTAL_CLOCK_SPEED;
    ...
    Status = EfipSdOmap4ResetController(Disk);
    if (!EFI_ERROR(Status)) {
...

{
    ...
    Disk = EFI_SD_OMAP4_FROM_THIS(This);
    Status = EfipSdOmap4ResetController(Disk);
    if (!EFI_ERROR(Status)) {
        Status = EfiSdInitializeController(Disk->Controller, FALSE);
    }
