...
PL11_CONTEXT EfiIntegratorDebugUart;
...

{
    ...
    EfiIntegratorDebugUart.UartBase = (VOID *)INTEGRATOR_UART_BASE;
    Status = EfipPl11ComputeDivisor(EFI_INTEGRATOR_UART_CLOCK_RATE,
                                    BaudRate,
                                    &(EfiIntegratorDebugUart.BaudRateInteger),
                                    &(EfiIntegratorDebugUart.BaudRateFraction));

    if (EFI_ERROR(Status)) {
        return Status;
    }

    return EfipPl11Initialize(&EfiIntegratorDebugUart);
}
...

{
    ...
    return EfipPl11Transmit(&EfiIntegratorDebugUart, Data, Size);
}
...

{
    ...
    return EfipPl11Receive(&EfiIntegratorDebugUart, Data, Size);
}
...

{
    ...
    return EfipPl11GetStatus(&EfiIntegratorDebugUart, ReceiveDataAvailable);
}
