...
Abstract:
    ...
    This module implements support for the serial device on the TI BeagleBone
    Black.
...
#define EFI_BBONE_SERIAL_FROM_THIS(_SerialIo)                 \
        (EFI_BBONE_SERIAL_CONTEXT *)((VOID *)(_SerialIo) -    \
                      ((VOID *)(&(((EFI_BBONE_SERIAL_CONTEXT *)0)->SerialIo))))

//
// ---------------------------------------------------------------- Definitions
//

#define EFI_BBONE_SERIAL_MAGIC 0x72655342 // 'reSB'

#define EFI_BBONE_DEFAULT_SERIAL_BAUD_RATE 115200
...
Structure Description:
    ...
    This structure describes the BeagleBone Black Serial I/O device context.

Members:

    Magic - Stores the magic constand EFI_BBONE_SERIAL_MAGIC.
...
typedef struct _EFI_BBONE_SERIAL_CONTEXT {
    UINT32 Magic;
    EFI_HANDLE Handle;
...
} EFI_BBONE_SERIAL_CONTEXT, *PEFI_BBONE_SERIAL_CONTEXT;

/*++

Structure Description:

    This structure defines the BeagleBone Black Serial I/O device path node.
...
typedef struct _EFI_BBONE_SERIAL_IO_DEVICE_PATH_NODE {
    VENDOR_DEVICE_PATH DevicePath;
    UINT32 ControllerBase;
} EFI_BBONE_SERIAL_IO_DEVICE_PATH_NODE, *PEFI_BBONE_SERIAL_IO_DEVICE_PATH_NODE;

/*++

Structure Description:

    This structure defines the BeagleBone Black Serial I/O device path form.
...
EFIAPI
EFI_STATUS
EfipBeagleBoneSerialReset (
    EFI_SERIAL_IO_PROTOCOL *This
    );

EFIAPI
EFI_STATUS
EfipBeagleBoneSerialSetAttributes (
    EFI_SERIAL_IO_PROTOCOL *This,
    UINT64 BaudRate,
...
EFIAPI
EFI_STATUS
EfipBeagleBoneSerialSetControlBits (
    EFI_SERIAL_IO_PROTOCOL *This,
    UINT32 Control
    );
...
EFIAPI
EFI_STATUS
EfipBeagleBoneSerialGetControlBits (
    EFI_SERIAL_IO_PROTOCOL *This,
    UINT32 *Control
    );
...
EFI_BBONE_SERIAL_IO_DEVICE_PATH EfiBeagleBoneSerialIoDevicePathTemplate = {
    {
        {
            {
                HARDWARE_DEVICE_PATH,
                HW_VENDOR_DP,
                sizeof(EFI_BBONE_SERIAL_IO_DEVICE_PATH_NODE)
            },
...
EFI_BBONE_SERIAL_CONTEXT EfiBeagleBoneSerialTemplate = {
    EFI_BBONE_SERIAL_MAGIC,
    NULL,
    NULL,
    {0},
    {
...

{
    ...
    VOID *ControllerBase;
    PEFI_BBONE_SERIAL_CONTEXT Device;
    PEFI_BBONE_SERIAL_IO_DEVICE_PATH DevicePath;
    EFI_STATUS Status;

    ControllerBase = (VOID *)AM335_UART_0_BASE;
    ...
    Status = EfiAllocatePool(EfiBootServicesData,
                             sizeof(EFI_BBONE_SERIAL_CONTEXT),
                             (VOID **)&Device);
    ...
    Status = EfiAllocatePool(EfiBootServicesData,
                             sizeof(EFI_BBONE_SERIAL_IO_DEVICE_PATH),
                             (VOID **)&DevicePath);

    if (EFI_ERROR(Status)) {
        goto BeagleBoneEnumerateSerialEnd;
    }

    EfiCopyMem(DevicePath,
               &EfiBeagleBoneSerialIoDevicePathTemplate,
               sizeof(EFI_BBONE_SERIAL_IO_DEVICE_PATH));

    DevicePath->Device.ControllerBase = (UINT32)ControllerBase;
    Device->DevicePath = (EFI_DEVICE_PATH_PROTOCOL *)DevicePath;
    Device->Mode.BaudRate = EFI_BBONE_DEFAULT_SERIAL_BAUD_RATE;
...

{
    ...
    Device = EFI_BBONE_SERIAL_FROM_THIS(This);
...

{
    ...
    Device = EFI_BBONE_SERIAL_FROM_THIS(This);
    if (BaudRate == 0) {
        BaudRate = EFI_BBONE_DEFAULT_SERIAL_BAUD_RATE;
    }
