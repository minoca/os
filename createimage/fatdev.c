...
Structure Description:
    ...
    This structure defines an I/O buffer for the image creation environment.
...
typedef struct _IMAGE_IO_BUFFER {
    PVOID Data;
    UINTN Size;
    UINTN CurrentOffset;
} IMAGE_IO_BUFFER, *PIMAGE_IO_BUFFER;
...

{
    ...
    PIMAGE_IO_BUFFER IoBuffer;

    IoBuffer = malloc(sizeof(IMAGE_IO_BUFFER) + Size);
    if (IoBuffer == NULL) {
        return NULL;
    }

    IoBuffer->Data = (PVOID)IoBuffer + sizeof(IMAGE_IO_BUFFER);
    IoBuffer->Size = Size;
...

{
    ...
    IoBuffer = malloc(sizeof(IMAGE_IO_BUFFER));
    if (IoBuffer == NULL) {
        return NULL;
    }
...

{
    ...
    if (Decrement == FALSE) {
        ((PIMAGE_IO_BUFFER)FatIoBuffer)->CurrentOffset += OffsetUpdate;

    } else {
        ((PIMAGE_IO_BUFFER)FatIoBuffer)->CurrentOffset -= OffsetUpdate;
    }

    ASSERT(((PIMAGE_IO_BUFFER)FatIoBuffer)->CurrentOffset <=
           ((PIMAGE_IO_BUFFER)FatIoBuffer)->Size);
...

{
    ...
    IoBuffer = (PIMAGE_IO_BUFFER)FatIoBuffer;
...

{
    ...
    PIMAGE_IO_BUFFER DestinationIoBuffer;
    PVOID SourceBuffer;
    PIMAGE_IO_BUFFER SourceIoBuffer;

    DestinationIoBuffer = (PIMAGE_IO_BUFFER)Destination;
    DestinationBuffer = DestinationIoBuffer->Data +
                        DestinationIoBuffer->CurrentOffset +
                        DestinationOffset;

    SourceIoBuffer = (PIMAGE_IO_BUFFER)Source;
...

{
    ...
    return ((PIMAGE_IO_BUFFER)FatIoBuffer)->Data;
}
...

{
    ...
    LONG ItemsRead;
    KSTATUS Status;
    ULONGLONG TranslatedBlockCount;
    PCI_VOLUME Volume;
    ...
    if (Volume->Partition != NULL) {
        TranslatedBlockCount = BlockCount;
        Status = PartTranslateIo(Volume->Partition,
                                 &BlockAddress,
        ...
        if (BlockCount != TranslatedBlockCount) {
    ...
    //
    // Seek the the correct place in the file.
    //

    File = Context->OutputFile;
    fseeko64(File, BlockSize * BlockAddress, SEEK_SET);

    assert(ftello64(File) == BlockSize * BlockAddress);
    ...
    ItemsRead = fread(Buffer, BlockSize, BlockCount, File);
...

{
    ...
    IoBuffer = (PIMAGE_IO_BUFFER)FatIoBuffer;
    Buffer = IoBuffer->Data + IoBuffer->CurrentOffset;
    ItemsWritten = fwrite(Buffer, BlockSize, BlockCount, File);
