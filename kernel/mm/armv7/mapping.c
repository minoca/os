...

{
    ...
    ULONG PotentialFirstIndex;
    ULONG PotentialSecondIndex;
    ULONG SecondIndex;
    volatile SECOND_LEVEL_TABLE *SecondLevelTable;
    PHYSICAL_ADDRESS SecondLevelTablePhysical;
    ULONG SelfMapIndex;
    ...
    if ((MmKernelFirstLevelTable == NULL) || (MmPageTables == NULL)) {
        return NULL;
    }
    ...
    FreePages = 0;
    PotentialFirstIndex = 0;
    PotentialSecondIndex = 0;
    for (FirstIndex = FLT_INDEX(KERNEL_VA_START);
         FirstIndex <= FLT_INDEX(MAX_ULONG);
         FirstIndex += 1) {
        ...
        if (FirstIndexDown == MmPageTablesFirstIndex) {
            FreePages = 0;
            PotentialFirstIndex = 0;
            PotentialSecondIndex = 0;
            continue;
        ...
        //
        // Check to see if the entry is present, and create page tables if it
        // is not.
        //
        ...
        if (FirstLevelTable[FirstIndex].Format == FLT_UNMAPPED) {
            ...
            if (MmTotalPhysicalPages == 0) {
                ...
                if (!KSUCCESS(Status)) {
                    goto EarlyAllocateMemoryEnd;
                }

            } else {
                SecondLevelTablePhysical = MmpAllocatePhysicalPages(1, 0);
                if (SecondLevelTablePhysical == INVALID_PHYSICAL_ADDRESS) {
                    goto EarlyAllocateMemoryEnd;
                }
            }

            //
            // Map the page into the staging area and zero it out. It is
            // guaranteed that cache operations complete before page table
            // changes become visible, meaning it is safe to clean the page
            // table region and then proceed to unmap it without an
            // intervening serialization.
            //

            OldRunLevel = KeRaiseRunLevel(RunLevelDispatch);
            SecondLevelTable = MmpMapPageTable(
                                         SecondLevelTablePhysical & ~PAGE_MASK);
            ...
            //
            // Create first level entries for all 4 page tables now.
            //

            for (LoopIndex = 0; LoopIndex < 4; LoopIndex += 1) {

                ASSERT(FirstLevelTable[FirstIndexDown + LoopIndex].Format ==
                       FLT_UNMAPPED);

                FirstLevelTable[FirstIndexDown + LoopIndex].Entry =
                         (((ULONG)SecondLevelTablePhysical) >> SLT_ALIGNMENT) +
                         LoopIndex;

                FirstLevelTable[FirstIndexDown + LoopIndex].Format =
                                                         FLT_COARSE_PAGE_TABLE;
            }

            MmpCleanPageTableCacheRegion(
                                     (PVOID)&(FirstLevelTable[FirstIndexDown]),
                                     sizeof(FIRST_LEVEL_TABLE) * 4);

            //
            // Also fix up the self map page table so that the new page table
            // is visible in the self map region.
            //

            SelfMapIndex = FirstIndex >> 2;
            SelfMapPageTable[SelfMapIndex] = MmSecondLevelInitialValue;
            SelfMapPageTable[SelfMapIndex].Entry =
                                 (ULONG)SecondLevelTablePhysical >> PAGE_SHIFT;
            ...
            SelfMapPageTable[SelfMapIndex].Access =
                                                         SLT_ACCESS_SUPERVISOR;

            SelfMapPageTable[SelfMapIndex].CacheAttributes =
                                                                SLT_WRITE_BACK;

            SelfMapPageTable[SelfMapIndex].Format = SLT_SMALL_PAGE;
            MmpCleanPageTableCacheLine(&(SelfMapPageTable[SelfMapIndex]));
            ArSerializeExecution();
        ...
        //
        // Scan through the second level table looking for a run that is large
        // enough.
        //

        for (SecondIndex = 0;
             SecondIndex < SLT_SIZE / sizeof(SECOND_LEVEL_TABLE);
             SecondIndex += 1) {
            ...
            if (PotentialFirstIndex == 0) {
                PotentialFirstIndex = FirstIndex;
                PotentialSecondIndex = SecondIndex;
            }
            ...
            if (FreePages == PageCount) {
                ...
                FirstIndex = PotentialFirstIndex;
                SecondIndex = PotentialSecondIndex;
                PagesMapped = 0;
                VirtualAddress = (PVOID)(FirstIndex << FLT_INDEX_SHIFT) +
                                        (SecondIndex << SLT_INDEX_SHIFT);

                NewAllocation = VirtualAddress;
                ...
                while (PagesMapped != PageCount) {
                    ...
                    SecondLevelTable[SecondIndex].CacheAttributes =
                                                                SLT_WRITE_BACK;
...

{
    ...
    ASSERT(MmKernelFirstLevelTable != NULL);

    FirstLevelTable = MmKernelFirstLevelTable;
    SecondLevelTable = NULL;
    SelfMapIndex = MmPageTablesFirstIndex;

    //
    // Look through every kernel first level table entry.
    //
    ...
    for (FirstIndex = FLT_INDEX(KERNEL_VA_START);
        ...
        //
        // Skip entries that aren't mapped.
        //
        ...
        if (ALIGN_RANGE_DOWN(FirstIndex, 4) == SelfMapIndex) {
            continue;
        }
        ...
        //
        // Mark each page mapped in the current page table.
        //
        ...
        for (SecondIndex = 0;
            ...
            VirtualAddress = (FirstIndex << FLT_INDEX_SHIFT) |
                             (SecondIndex << SLT_INDEX_SHIFT);
