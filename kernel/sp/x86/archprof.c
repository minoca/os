...
Abstract:
    ...
    This module implements system profiling routines specific to the x86
    architecture.
...

{
    ...
    InstructionPointer = (PULONG)TrapFrame->Eip;
    ...
    if (IS_FUNCTION_PROLOGUE(*InstructionPointer) != FALSE) {
        ...
        if (TrapFrame->Esp < (ULONG)KERNEL_VA_START) {
            goto GetKernelStackDataEnd;
        }

        ReturnAddress = (PVOID)*((PULONG)TrapFrame->Esp);
        if (ReturnAddress == 0) {
            goto GetKernelStackDataEnd;
        }
    ...
    BasePointer = (PULONG)TrapFrame->Ebp;
    ...
    while (BasePointer != 0) {
        ...
        BasePointer = (PULONG)*BasePointer;
        if (BasePointer == 0) {
            break;
        }
