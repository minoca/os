...

{
    ...
    //
    // Otherwise stop any changes to the mount point tree and search for a
    // child mount point that has the given path entry as the mount point
    // entry. Don't acquire or release mount point references here. The caller
    // is most likely holding a file object lock.
    //
    ...
    while (CurrentEntry != &(Parent->ChildListHead)) {
        ...
        //
        // If the path entry matches the mount point, get the target's file ID.
        //

        if (MountPoint->MountEntry == PathEntry) {
            TargetFileObject = MountPoint->TargetEntry->FileObject;
            *FileId = TargetFileObject->Properties.FileId;
