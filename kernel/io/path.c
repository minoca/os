...

{
    ...
    //
    // Get the path points for this process's current directory and root
    // directory.
    //
    ...
    if (Paths->Root.PathEntry != NULL) {
        IO_COPY_PATH_POINT(&RootCopy, &(Paths->Root));
        IO_PATH_POINT_ADD_REFERENCE(&RootCopy);
        Root = &RootCopy;
    }

    KeReleaseQueuedLock(Paths->Lock);

    //
    // Create a path to the current directory relative to the root.
    //
